...
if($pagenow == 'index.php' || $pagenow == 'themes.php'){
	if ( isset( $_GET['hide_admin_notice'] ) ) {
			update_option('one_page_business_hide_admin_notice', 'hide-notice');
	} else {
		$one_page_business_notice = get_option('one_page_business_hide_admin_notice', '');
		if ($one_page_business_notice != 'hide-notice' || $one_page_business_notice == '') {	
...
function one_page_business_admin_notice_info() {
    ...
    $message = __( ' See Appearance -> Customize -> Theme Options to customize', 'one-page-business' );
 	$dismiss = __( 'Dismiss', 'one-page-business');
    printf( '<div class="%1$s"> <p> 
	
	<a class="tsp-btn-get-started button button-primary button-hero tsp-button-padding" href="#" data-name="" data-slug="" >'.esc_html__("Demo install and complete imstallation... ","one-page-business").'</a>	
